<?php
session_start();
function verifyAlreadyInsertedRes(int $bureau, int $candidat){
   ...
   if(isset($_FILES['image'])){
      ...
      else
      {
          ...
          echo "<script> alert('la taille de l'image est trop grande');</script>";
              ...
              $connexion=$connect->open();
              foreach($panier as $p){
               $reponse=verifyAlreadyInsertedRes($p['id_bureau'],$p['num_cand']);
               if($reponse===0){
                  try{
...
function storeResultPanier()
{
   ...
   if($reponse===0){
      try{
      $requete=$connexion->prepare('INSERT INTO panier(nbre_voix, id_bureau,num_cand,user) values(:voix,:bureau,:cand,:user)');
      $requete->execute([
         
         'voix'=>intval($_POST['voix']),
         'bureau'=>intval($_POST['bureau']),
         'cand'=>intval($_POST['candidat']),
         'user'=>intval($_SESSION['utilisateur'])
         
      ]);
      return "Résultat enregistré avec succès";
      $_SESSION['bureau']=getBureau()[0];
    ...
    return "une erreur s'est produite dans l'enregistrement du résultat. Réessayez svp: <br>".$e;
      ...
      echo "<script> alert('Déjà dans le panier');</script>";
      $_SESSION['bureau']=getBureau()[0];
...
function getBureau(){
   $connect=new Connexion();
   $connexion=$connect->open();
   $requete=$connexion->prepare("select libelle_bureau from bureau where id_bureau=:id");
   $requete->execute([
      'id'=>intval($_POST['bureau'])
   ]);
 ...
 function getResultPanier(){
   $connect=new Connexion();
   $connexion=$connect->open();
   $requete=$connexion->prepare("select cand.nom_cand nom,cand.prenom_cand prenom ,p.nbre_voix voix,p.num_cand cand,b.libelle_bureau bureau
   from candidat cand
    inner join panier p
    on cand.numero=p.num_cand
    inner join bureau b
    on b.id_bureau=p.id_bureau");
   $requete->execute([
...
function deleteCandPanier(){
   $connect=new Connexion();
   $connexion=$connect->open();
   $requete=$connexion->prepare("delete from panier where num_cand=:id");
   $requete->execute([
      'id'=>intval($_GET['del'])
   ]);
   // return $users=$requete->fetchAll();
...
function deletePanier(){
   $connect=new Connexion();
   $connexion=$connect->open();
   $requete=$connexion->prepare("delete from panier where user=:id");
   $requete->execute([
      'id'=>intval($_SESSION['utilisateur'])
   ]);
   // return $users=$requete->fetchAll();
...
function utilisateur()
{
   $connect=new Connexion();
   $connexion=$connect->open();
   $requete=$connexion->prepare("select u.nom_user, u.prenom, circ.id_circ circonscription,ct.id_centre centre  from utilisateur u inner join centre ct on u.id_centre=ct.id_centre inner join circonscription circ on ct.id_circ=circ.id_circ where u.id_user=:id");
   $requete->execute([
      'id'=>intval($_SESSION['utilisateur'])
   ]);
   return $users=$requete->fetchAll();
...
function storeCentre()
{
   ...
   $connexion=$connect->open();
   try{
      $requete=$connexion->prepare('insert into centre(libelle_centre, id_circ)
       values(:nom,:id)');
      $requete->execute([
...
function temoins()
{
   $connect=new Connexion();
   $connexion=$connect->open();
   $requete=$connexion->prepare("select u.nom_user, u.prenom,u.sexe,u.telephone,u.mail, circ.id_circ circonscription,ct.id_centre centre,ct.libelle_centre  from utilisateur u inner join centre ct on u.id_centre=ct.id_centre inner join circonscription circ on ct.id_circ=circ.id_circ where u.type=:id");
   $requete->execute([
      'id'=>0
   ]);
   return $users=$requete->fetchAll();
...
function getBureaux(){
   ...
   $connect=new Connexion();
   $connexion=$connect->open();
   $requete=$connexion->prepare("select b.id_bureau bur, b.libelle_bureau bureau 
   from bureau b inner join centre ct on b.id_centre=ct.id_centre
   inner join circonscription circ on ct.id_circ=circ.id_circ where circ.id_circ=:id ");
   $requete->execute([
      'id'=>intval($id),
   ]);
   return $categories=$requete->fetchAll();
...
function verifyAlreadyInserted(){
   $connect=new Connexion();
   $connexion=$connect->open();
   $requete=$connexion->prepare(" select * from panier where id_bureau=:bur and num_cand=:num");
   $requete->execute([
